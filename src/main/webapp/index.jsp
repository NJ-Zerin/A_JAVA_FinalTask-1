<%@ page isELIgnored="false" %>
<html>
<body style="margin: 0; padding: 0;">
<nav style="background-color: #05095B; overflow: hidden; font-size: 18px; padding: 1%">
  <a href="${pageContext.request.contextPath}" style="float: left; color: white;  padding: 14px 16px; text-decoration: none;">Home</a>
  <a href="${pageContext.request.contextPath}/create" style="float: left; color: white;  padding: 14px 16px; text-decoration: none;">Create Student</a>
  <a href="" style="float: left; color: white;  padding: 14px 16px; text-decoration: none;">All Students</a>
</nav>
<div style="text-align: center; margin-top: 15%">
<h1 style="font-size: 50px">Hello !!! </h1>
<h2 style="font-size: 40px">I am Nusrat Jahan Zerin</h2>
<div>
<footer style="background-color: #05095B;text-align: center; color: white; margin-top: 12%; padding: 3%"><span style="margin-top: 12%">All reserved &copy 2023</span></footer>

</body>
</html>
